...
def remove_prefix(text, prefix="Pytorch_model_"):
...
model_List=['Pytorch_model_LSTM','Pytorch_model_GRU','keras_lstm_model','RandomForest_model','SVM_model']
...
for model_name in model_List:
    ...
    df_columns.append(column_name)  # 添加列名到列表

    df_AUCROC = pd.DataFrame(data=None, index=experiment_params, columns=df_columns)
    df_AUCPR = pd.DataFrame(data=None, index=experiment_params, columns=df_columns)
    df_time_fit = pd.DataFrame(data=None, index=experiment_params, columns=df_columns)
    df_time_inference = pd.DataFrame(data=None, index=experiment_params, columns=df_columns)

# 每一种模型都遍历一遍
for model_name in model_List:
    column_name=model_name
    if model_name.startswith("Pytorch_model_"):
        algorithm_type = remove_prefix(model_name)
